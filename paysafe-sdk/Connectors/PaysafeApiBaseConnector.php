...
abstract class PaysafeApiBaseConnector
{
    ...
    ): PaysafeApiResult
    {
        ...
        // Ensure methods that commonly require a body always send one
        if (in_array($method, [self::METHOD_POST, self::METHOD_PUT, self::METHOD_DELETE], true)) {
            $hasPayload   = !empty($data);
            $args['body'] = $hasPayload ? wp_json_encode($data) : ''; // '' => Content-Length: 0

            if ($hasPayload) {
                $this->logApiDebug("call_paysafe_api: $method data sent", $data);
            }
        }

        $curlResponseRaw = wp_remote_request(
            $apiUrl,
            $args
        );
