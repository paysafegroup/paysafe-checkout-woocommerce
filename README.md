...
Besides card payments, the plugin supports following payment methods (APMs):
...
1. The first case is when the payment is only authorized, in which a Void transaction is issued to cancel the
   authorization.
2. The second case is canceling the settlement for authorized and captured transactions that have not yet been
...
https://github.com/paysafegroup/paysafe-checkout-woocommerce/blob/main/Paysafe-Checkout-for-WooCommerce-Merchant-Guide.pdf
