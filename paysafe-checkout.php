 ...
 * Plugin URI:          https://developer.paysafe.com/en/api-docs/
 * Description:         Accept payments on your WooCommerce store while minimising your security and PCI compliance requirements.
...
define('PAYSAFE_WOO_VERSION', '1.0.0');
...
function paysafe_woo_payment_gateways(): array
{
    ...
    if (!$paysafe_is_settings_page && WC_Gateway_Paysafe_Base::ALLOW_APPLE_PAY_SUPPORT) {
        $gateways[] = 'WC_Gateway_Paysafe_Apple_Pay';
    }
    if (!$paysafe_is_settings_page && WC_Gateway_Paysafe_Base::ALLOW_LPM_PAY_SUPPORT) {
        $gateways[] = 'WC_Gateway_Paysafe_Skrill';
        $gateways[] = 'WC_Gateway_Paysafe_Neteller';
        $gateways[] = 'WC_Gateway_Paysafe_Card';
    }
    if (!$paysafe_is_settings_page && WC_Gateway_Paysafe_Base::ALLOW_LPM_PAYSAFECASH_SUPPORT) {
