...
define('PAYSAFE_WOO_VERSION', '4.3.5');
...
function paysafe_woo_init(): void
{
    ...
    if (! class_exists('WC_Gateway_Paysafe_Paypal')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-paypal.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Sightline')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-sightline.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Vipprefered')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-vippreferred.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Eft')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-eft.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Ach')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-ach.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Paybybank')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-paybybank.php';
    }
    if (! class_exists('WC_Gateway_Paysafe_Venmo')) {
        require_once PAYSAFE_WOO_PLUGIN_PATH . '/includes/gateway/class-wc-gateway-paysafe-venmo.php';
    }
    ...
    // Register admin REST routes exactly once per request
    add_action('rest_api_init', 'paysafe_woo_register_rest_routes');
}

/**
 * Register Paysafe admin REST API routes.
 * Called once via rest_api_init, not from each gateway constructor.
 *
 * @since 4.3.5
 */
function paysafe_woo_register_rest_routes(): void
{
            ...
            use($paysafe_is_settings_page)
        {
            ...
            if (!$paysafe_is_settings_page) {
                ...
                if (WC_Gateway_Paysafe_Base::ALLOW_LPM_PAYPAL) {
	                $payment_method_registry->register(
		                new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Paypal::PAYMENT_GATEWAY_CODE )
	                );
				}
                if (WC_Gateway_Paysafe_Base::ALLOW_LPM_SIGHTLINE ) {
	                $payment_method_registry->register(
		                new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Sightline::PAYMENT_GATEWAY_CODE )
	                );
				}
                if (WC_Gateway_Paysafe_Base::ALLOW_LPM_VIPPREFERRED ) {
	                $payment_method_registry->register(
		                new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Vippreferred::PAYMENT_GATEWAY_CODE)
	                );
				}
                if ( WC_Gateway_Paysafe_Base::ALLOW_LPM_EFT ) {
	                $payment_method_registry->register(
		                new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Eft::PAYMENT_GATEWAY_CODE)
	                );
				}
                if (WC_Gateway_Paysafe_Base::ALLOW_LPM_ACH ) {
	                $payment_method_registry->register(
		                new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Ach::PAYMENT_GATEWAY_CODE)
	                );
				}
	            if ( WC_Gateway_Paysafe_Base::ALLOW_LPM_PAY_BY_BANK ) {
		            $payment_method_registry->register(
			            new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Paybybank::PAYMENT_GATEWAY_CODE)
	                );
				}
				if (WC_Gateway_Paysafe_Base::ALLOW_LPM_VENMO) {
					$payment_method_registry->register(
						new WC_Gateway_Paysafe_Blocks_Support( WC_Gateway_Paysafe_Venmo::PAYMENT_GATEWAY_CODE )
					);
				}
            }
        });
...
function paysafe_woo_payment_gateways(): array
{
    ...
    if (!$paysafe_is_settings_page) {
		...
		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_PAYPAL) {
			$gateways[] = 'WC_Gateway_Paysafe_Paypal';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_SIGHTLINE) {
			$gateways[] = 'WC_Gateway_Paysafe_Sightline';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_VIPPREFERRED) {
			$gateways[] = 'WC_Gateway_Paysafe_Vippreferred';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_EFT) {
			$gateways[] = 'WC_Gateway_Paysafe_Eft';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_ACH) {
			$gateways[] = 'WC_Gateway_Paysafe_Ach';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_PAY_BY_BANK) {
			$gateways[] = 'WC_Gateway_Paysafe_Paybybank';
		}

		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_VENMO) {
			$gateways[] = 'WC_Gateway_Paysafe_Venmo';
		}
    }
