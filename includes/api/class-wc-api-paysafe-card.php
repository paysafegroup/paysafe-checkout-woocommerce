...
class PaysafeApiCardPluginConnector extends PaysafeApiBasePluginConnector
{
    ...
    public function voidPayment(string $paymentId, array $data): array
    {
        try {
            ...
            throw new PaysafeException(
                esc_html(__('Void payment failed', 'paysafe-checkout')),
                (int)$e->getCode()
            );
    ...
    public function settlePayment(string $paymentId, array $data): array
    {
        try {
            ...
            // phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new PaysafeException(
                esc_html(__('Settle payment failed', 'paysafe-checkout')),
                (int)$e->getCode()
            );
            // phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
        } catch (\Exception $e) {
    ...
    public function refundPayment(string $settlementId, array $data): array
    {
        try {
            ...
            // phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new PaysafeException(
                esc_html(__('Refund payment failed', 'paysafe-checkout')),
                (int)$e->getCode()
            );
            // phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
        } catch (\Exception $e) {
    ...
    public function handleProcessPayment(array $data): array
    {
        try {
            ...
            // phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new PaysafeException(
                esc_html(__('Process payment failed', 'paysafe-checkout')),
                (int)$e->getCode()
            );
            // phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
        } catch (\Exception $e) {
    ...
    public function handleVerifyPayment(array $data): array
    {
        try {
            ...
            // phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new PaysafeException(
                esc_html(__('Verify payment failed', 'paysafe-checkout')),
                (int)$e->getCode()
            );
            // phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
        } catch (\Exception $e) {
    ...
    public function getPaysafeCustomerDataByMid(array $data): array
    {
        try {
            ...
            $this->logError(
                'Paysafe Checkout: Get customer data by merchant customer id exception ');
    ...
    public function getPaymentHandleByMerchantReference(array $data): array
    {
        try {
            ...
            $this->logError('Paysafe Checkout: Get payment handle by reference exception ' . $e->getMessage());
    ...
    public function getPaysafeSettlementData(string $settlementId): array
    {
        try {
            ...
            $this->logError('Paysafe Checkout: Get Paysafe settlement data by settlement ID exception ' . $e->getMessage());
	...
	public function getPaysafePaymentData(string $paymentId): array
	{
		try {
			...
			$this->logError('Paysafe Checkout: Get Paysafe payment data by payment ID exception ' . $e->getMessage());
	...
	public function handleSuph2MuphExchange(string $paysafe_customer_id, array $data): array
	{
		try {
			...
			// phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
			throw new PaysafeException(
				esc_html(__('Process exchange single use token to multi use failed', 'paysafe-checkout')),
				(int)$e->getCode()
			);
			// phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
		} catch (\Exception $e) {
	...
	/**
	 * Get Paysafe payment handle data by merchant ref number
	 *
	 ...
	 * @throws PaysafeException
	 */
	public function getPaysafePaymentHandleData(string $merchantRefNum): array
	{
		try {
			$paysafeApiResult = $this->connector->getPaysafePaymentHandleData($merchantRefNum);

			return $paysafeApiResult->getData();
		} catch (PaysafeApiException $e) {
			$this->logError('Paysafe Checkout: Get payment handle data exception ' . $e->getMessage(),
				$e->getAdditionalData());

			// phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped
			throw new PaysafeException(
				esc_html(
					__('Get payment handle data failed', 'paysafe-checkout')
				),
				(int)$e->getCode()
			);
			// phpcs:enable WordPress.Security.EscapeOutput.ExceptionNotEscaped
