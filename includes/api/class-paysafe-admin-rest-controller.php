...
class Paysafe_Admin_REST_Controller
{
    ...
    /**
     * Sanitize a payment methods tree before storage.
     * Only whitelisted keys with string/bool values are kept.
     *
     * @param mixed $tree
     * @return array
     */
    private static function sanitize_payment_methods_tree( $tree ): array
    {
        if ( ! is_array( $tree ) ) {
            return [];
        }
        ...
        $clean = [];
        foreach ( $tree as $method_key => $method_node ) {
            $method_key = sanitize_text_field( (string) $method_key );
            if ( ! preg_match( '/^[A-Z0-9_]{1,32}$/', $method_key ) ) {
                continue;
            }

            if ( ! is_array( $method_node ) ) {
                continue;
            }

            $clean_node = [
                'name'      => sanitize_text_field( (string) ( $method_node['name']      ?? '' ) ),
                'available' => (bool) ( $method_node['available'] ?? false ),
                'accounts'  => [],
            ];

            foreach ( ( $method_node['accounts'] ?? [] ) as $account_id => $account ) {
                $account_id = sanitize_text_field( (string) $account_id );
                if ( ! is_array( $account ) ) {
                    continue;
                }
                $clean_node['accounts'][ $account_id ] = [
                    'account_id'      => sanitize_text_field( (string) ( $account['account_id']      ?? '' ) ),
                    'processor'       => sanitize_text_field( (string) ( $account['processor']       ?? '' ) ),
                    'mcc'             => sanitize_text_field( (string) ( $account['mcc']             ?? '' ) ),
                    'mcc_description' => sanitize_text_field( (string) ( $account['mcc_description'] ?? '' ) ),
                ];
            }

            $clean[ $method_key ] = $clean_node;
        }
        ...
        foreach ($data as $key => $value) {
            if ($key === 'payment_methods_sandbox' || $key === 'payment_methods_live') {
                $settings[$key] = is_array($value)
                    ? array_map( [ self::class, 'sanitize_payment_methods_tree' ], $value )
                    : [];
                continue;
            }
