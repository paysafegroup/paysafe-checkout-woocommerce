...
class WC_Gateway_Paysafe_Paybybank extends WC_Gateway_Paysafe_Base {
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('Pay by Bank', 'paysafe-checkout');
        } else {
            $this->title = __('Pay by Bank', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with Pay by Bank', 'paysafe-checkout');
        }
    ...
    public function payment_fields() {
	    ...
	    $this->show_paysafe_paybybank_form();
    }
	 ...
	 * @return void
	 */
	private function show_paysafe_paybybank_form(): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-paybybank-payment-form.php';
