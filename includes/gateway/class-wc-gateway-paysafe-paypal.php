...
class WC_Gateway_Paysafe_Paypal extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'paysafepaypal';
    const PAYMENT_TYPE_CODE = 'PAYPAL';

    const PAYMENT_ICON_FILE = 'paysafe-paypal.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('PayPal', 'paysafe-checkout');
        } else {
            $this->title = __('PayPal', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with PayPal', 'paysafe-checkout');
        }
	 ...
	 * @return void
	 */
	private function show_paysafe_paypal_form(): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-paypal-payment-form.php';
