...
class WC_Gateway_Paysafe extends WC_Gateway_Paysafe_Base {
    ...
    public function __construct()
    {
        ...
        if (empty($this->description)) {
            $this->description = __('Easy and secure card payments', 'paysafe-checkout');
        }
    ...
    ): array
    {
        ...
        if (!isset($settings['account_id_' . $currency_code . '_' . $payment_code_apple_pay])) {
    ...
    ): array
    {
        ...
        // if this is the first time the merchant saves this, add the default value to save tokens
        if (!isset($settings['save_tokens_' . $currency_code . '_' . $payment_code])) {
            $settings['save_tokens_' . $currency_code . '_' . $payment_code]
    ...
    public function payment_fields() {
        ...
        if ( self::ALLOW_TOKENIZATION_SUPPORT && $this->supports( 'tokenization' ) && is_checkout() ) {
            $this->saved_payment_methods();
        }

        if ($this->get_integration_type() === self::PAYMENT_INTEGRATION_PAYSAFE_JS) {
    ...
    public function delete_paysafe_token($token_id, $token)
    {
        ...
        if (!$customer_id || !$paysafe_token) {
            $this->log_error(PAYSAFE_DELETE_TOKEN_TEXT . ' failed - Customer ID or Paysafe token not found');

            return;
        }
        $paysafe_token_id = $this->get_paysafe_token_id($customer_id, $paysafe_token);
        ...
        try {
            ...
            $result = $api_connector->deletePaysafeToken($customer_id, $paysafe_token_id);
        } catch (PaysafeException $e) {
            $this->log_error(
     ...
     * @return string|null
     */
    private function get_paysafe_token_id(string $customer_id, string $paysafe_token): ?string
    {
        if (!defined('PAYSAFE_GET_PAYSAFE_TOKEN_TEXT')) {
            define('PAYSAFE_GET_PAYSAFE_TOKEN_TEXT',
                'Paysafe Checkout: [Customer ID ' . $customer_id . '] Get Paysafe Token');
        }

        if (!$customer_id || !$paysafe_token) {
            $this->log_error(PAYSAFE_GET_PAYSAFE_TOKEN_TEXT
                . ' failed - Customer ID or Paysafe token not found');
        ...
        try {
            ...
            foreach($result['paymentHandles'] ?? [] as $payment_handle) {
                if($payment_handle['paymentHandleToken'] ?? '' == $paysafe_token){
                    return $payment_handle['id'] ?? null;
                }
            }
        } catch (PaysafeException $e) {
            $this->log_error(
                PAYSAFE_GET_PAYSAFE_TOKEN_TEXT . ' - Paysafe Exception "'
                . $e->getMessage() . '"', array_merge($result ?? [], $e->getAdditionalData()));
        } catch (\Exception $e) {
            $this->log_error(PAYSAFE_GET_PAYSAFE_TOKEN_TEXT . ' - Exception "' . $e->getMessage() . '"');
        }
    ...
    private function add_payment_method_form(): void
    {
        ...
        $billing_details = [
            ...
            'street' => trim($current_customer->get_billing_address_1()
                    . ' class-wc-gateway-paysafe.php' . $current_customer->get_billing_address_2()) ?? '',
        ];
                ...
                if (
                    ...
                    return $this->save_paysafe_customer_token(
                        $payment_handle_data['paymentHandleToken'],
                            $payment_handle_data['card'] ?? [], self::PAYMENT_TYPE_CODE);
                }
            }
        ...
        try {
            ...
            return $this->save_paysafe_customer_token(
                $payment_handle_token,
                    $result['card'] ?? [], self::PAYMENT_TYPE_CODE);
