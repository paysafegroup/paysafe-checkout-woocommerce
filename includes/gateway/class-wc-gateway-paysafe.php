...
class WC_Gateway_Paysafe extends WC_Gateway_Paysafe_Base {
    ...
    const PAYMENT_ICON_FILE = 'card-brands.svg';
    ...
    public function __construct()
    {
		...
		add_action('admin_enqueue_scripts', [$this, 'enqueue_admin_react_app'], 100);
     ...
     * @return bool
     */
    private function is_new_public_key_valid(array $data): bool
    {
        return $this->is_new_key_valid($data, true);
    }
     ...
     * @return bool
     */
    private function is_new_private_key_valid(array $data): bool
    {
        return $this->is_new_key_valid($data, false);
    }
    ...
    /**
     * Test new keys validity
     *
     * @param bool $use_public_key
     *
     * @return bool
     */
    private function is_new_key_valid(array $data, bool $use_public_key = false): bool
    {
        try {
            // initiate the paysafe general connector
            $api_connector = new PaysafeApiGeneralPluginConnector($data);
            return $api_connector->testNewKey($data, $use_public_key);
        } catch (\Exception $e) {
            $this->log_error('Paysafe Checkout: Public key invalid "' . $e->getMessage() . '"');
        }
    ...
    /*
     * Same function as process_admin_options() but for async requests
     */
    public function test_connection_credentials(array $data): array
    {
        if ( ! ($this->is_new_public_key_valid($data) && $this->is_new_private_key_valid($data))) {
            $error = __('Your API keys are invalid.', 'paysafe-checkout');
            return [
                'success' => false,
                'message' => $error,
            ];
        ...
        try {
            ...
            $new_settings = $this->get_updated_paysafe_payment_methods_options(
                $payment_methods_response['paymentMethods'] ?? [],
                $data // overlay fresh creds into newSettings
            );

            return [
                'success' => true,
                'message' => __('✅ Connection successful! Please check your payment methods.', 'paysafe-checkout'),
                'paymentMethods' => $payment_methods_response['paymentMethods'] ?? [],
            ...
            return [
                'success' => false,
                'message' => sprintf(
                    /* translators: Paysafe error. 1: message, 2: error code. */
                    __('Paysafe error: %1$s %2$s', 'paysafe-checkout'),
                    $e->getMessage(),
                    trim(($additional_data['error_code'] ?? '') . ' ' . ($additional_data['error_message'] ?? ''))
                ),
            ];
        } catch (\Exception $e) {
            return [
                'success' => false,
                'message' => __('Unexpected error: ', 'paysafe-checkout') . $e->getMessage(),
            ];
        }
    }
    ...
    ): array
    {
        ...
        if (!isset($settings['account_id_' . $currency_code . '_' . $payment_code_apple_pay . $this->test_live_suffix()]) || empty($settings['account_id_' . $currency_code . '_' . $payment_code_apple_pay . $this->test_live_suffix()])) {
            $settings['account_id_' . $currency_code . '_' . $payment_code_apple_pay . $this->test_live_suffix()]
	...
	): array
	{
		...
		// save the default account ID for the merchant, so that he can use it right away
		if (!isset($settings['account_id_' . $currency_code . '_' . $payment_code_google_pay . $this->test_live_suffix()]) || empty($settings['account_id_' . $currency_code . '_' . $payment_code_google_pay . $this->test_live_suffix()])) {
			$settings['account_id_' . $currency_code . '_' . $payment_code_google_pay . $this->test_live_suffix()]
				= $payment_method['accountId'];
		}
    ...
    ): array
    {
        ...
        if (
            ...
            || !in_array($payment_code, [
                ...
                WC_Gateway_Paysafe_Eft::PAYMENT_TYPE_CODE,
                WC_Gateway_Paysafe_Ach::PAYMENT_TYPE_CODE,
	            WC_Gateway_Paysafe_Paypal::PAYMENT_TYPE_CODE,
	            WC_Gateway_Paysafe_Sightline::PAYMENT_TYPE_CODE,
	            WC_Gateway_Paysafe_Vippreferred::PAYMENT_TYPE_CODE,
	            WC_Gateway_Paysafe_Paybybank::PAYMENT_TYPE_CODE,
	            WC_Gateway_Paysafe_Venmo::PAYMENT_TYPE_CODE,
            ])
        ) {
            return $settings;
        }
        ...
        // save the default account ID for the merchant, so that he can use it right away
        if (
            !isset($settings['account_id_' . $currency_code . '_' . $payment_code . $this->test_live_suffix()])
            || (
                (int)$payment_method['accountId']
                && !$settings['account_id_' . $currency_code . '_' . $payment_code . $this->test_live_suffix()]
            )
        ) {
            $settings['account_id_' . $currency_code . '_' . $payment_code . $this->test_live_suffix()] = $payment_method['accountId'];
        }

        // if this is the first time the merchant has saved this, add the default value to save tokens
        if (!isset($settings['save_tokens_' . $payment_code . $this->test_live_suffix()])) {
            $settings['save_tokens_' . $payment_code . $this->test_live_suffix()]
                = self::SAVE_TOKEN_DEFAULT_VALUE;
        }
     ...
     * @return void
     */
    private function update_paysafe_payment_methods_options(array $payment_methods, $display_errors = true): void
    {
        $this->log_debug(
        ...
        if (!empty($payment_methods)) {
            $settings['payment_method_' . $currency_code . '_' . WC_Gateway_Paysafe::PAYMENT_TYPE_CODE . $this->test_live_suffix()] = 'yes';
        }

        // set all unavailable payment methods to NO
        foreach ($settings['payment_methods'][$currency_code] as $payment_code => $payment_method) {
            if (!($payment_method['available'])) {
                $settings['payment_method_' . $currency_code . '_' . $payment_code . $this->test_live_suffix()] = 'no';
            }
        }
    ...
    private function get_updated_paysafe_payment_methods_options(array $payment_methods, array $data = []): array
    {
        // Start from current settings but overlay the just-entered creds
        $settings = $this->get_paysafe_settings();
        foreach (['test_mode','test_public_api_key','test_private_api_key','live_public_api_key','live_private_api_key'] as $k) {
        ...
        // correct shape and nuke any stale methods for this currency
        $settings['payment_methods'] = [
            $currency_code => [],
        ];

        // Build the tree purely from what the API just returned
        $settings = $this->handle_update_payment_methods_settings($payment_methods, $settings);

        // Force-enable CARD if anything came back
        if (!empty($payment_methods)) {
            $settings['payment_method_' . $currency_code . '_' . self::PAYMENT_TYPE_CODE . $this->test_live_suffix()] = 'yes';
        }

        // Mark unavailable methods as "no"
        foreach ($settings['payment_methods'][$currency_code] as $payment_code => $pm) {
            if (empty($pm['available'])) {
                $settings['payment_method_' . $currency_code . '_' . $payment_code . $this->test_live_suffix()] = 'no';
            }
        }

        if (isset($settings['webhook_url' . $this->test_live_suffix()])) {
            $settings['webhook_url' . $this->test_live_suffix()] = '';
        }
        if (isset($settings['webhook_secret_key' . $this->test_live_suffix()])) {
            $settings['webhook_secret_key' . $this->test_live_suffix()] = '';
        }
    ...
    public function delete_paysafe_token($token_id, $token)
    {
        ...
        if (!$token || !$current_user_id || $token->get_user_id() !== $current_user_id || $token->get_gateway_id() !== $this->id) {
