...
class WC_Gateway_Paysafe_Sightline extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'sightline';
    const PAYMENT_TYPE_CODE = 'SIGHTLINE';

    const PAYMENT_ICON_FILE = 'paysafe-sightline.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('Play+ (Sightline)', 'paysafe-checkout');
        } else {
            $this->title = __('Play+ (Sightline)', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with Play+ (Sightline)', 'paysafe-checkout');
        }
    ...
    public function payment_fields() {
	    ...
	    $this->show_paysafe_sightline_form();
    }
	 ...
	 * @return void
	 */
	private function show_paysafe_sightline_form(): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-sightline-payment-form.php';
