...
class WC_Gateway_Paysafe_Venmo extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'venmo';
    const PAYMENT_TYPE_CODE = 'VENMO';

    const PAYMENT_ICON_FILE = 'paysafe-venmo.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('Venmo', 'paysafe-checkout');
        } else {
            $this->title = __('Venmo', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with Venmo', 'paysafe-checkout');
        }
	 ...
	 * @return void
	 */
	private function show_paysafe_venmo_form(): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-venmo-payment-form.php';
