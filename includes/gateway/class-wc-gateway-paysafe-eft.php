...
class WC_Gateway_Paysafe_Eft extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'eft';
    const PAYMENT_TYPE_CODE = 'EFT';

    const PAYMENT_ICON_FILE = 'paysafe-eft.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('Eft', 'paysafe-checkout');
        } else {
            $this->title = __('Eft', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with Eft', 'paysafe-checkout');
        }
