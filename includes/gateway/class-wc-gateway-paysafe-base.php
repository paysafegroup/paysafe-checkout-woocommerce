                ...
                if ($payment_handle_data && isset($payment_handle_data['paymentHandles'])) {
                    foreach ($payment_handle_data['paymentHandles'] as $payment_handle_item_data) {
                        if ($payment_handle_item_data['paymentHandleToken'] === $payment_handle_token) {
                            // GOT IT !!!
                            $payment_method_key1 = $payment_method_key2 = '';
                            if ($paysafe_payment_method === WC_Gateway_Paysafe_Apple_Pay::PAYMENT_TYPE_CODE) {
                                $payment_method_key1 = 'applePay';
                                $payment_method_key2 = 'applePayPaymentToken';
                            }
                            if ($paysafe_payment_method === WC_Gateway_Paysafe_Google_Pay::PAYMENT_TYPE_CODE) {
                                $payment_method_key1 = 'googlePay';
                                $payment_method_key2 = 'googlePayPaymentToken';
                            }

                            if ($payment_method_key1
                                && isset($payment_handle_item_data[$payment_method_key1][$payment_method_key2])) {
                                $phidi =
                                        $payment_handle_item_data[$payment_method_key1][$payment_method_key2];
                                $billing_address_data =
                                        $phidi['paymentMethodData']['info']['billingAddress'] ?? [];
                                $shipping_address_data = $phidi['shippingAddress'] ?? [];

                                // update billing address
                                if ($billing_address_data['name'] ?? null) {
                                    $order->set_billing_first_name( $billing_address_data['name'] );
                                }
                                if ($billing_address_data['address1'] ?? null) {
                                    $order->set_billing_address_1( $billing_address_data['address1'] );
                                }
                                if ($billing_address_data['address2'] ?? null) {
                                    $order->set_billing_address_2( $billing_address_data['address2'] );
                                }
                                if ($billing_address_data['locality'] ?? null) {
                                    $order->set_billing_city( $billing_address_data['locality'] );
                                }
                                if ($billing_address_data['administrativeArea'] ?? null) {
                                    $order->set_billing_state( $billing_address_data['administrativeArea'] );
                                }
                                if ($billing_address_data['postalCode'] ?? null) {
                                    $order->set_billing_postcode( $billing_address_data['postalCode'] );
                                }
                                if ($billing_address_data['countryCode'] ?? null) {
                                    $order->set_billing_country( $billing_address_data['countryCode'] );
                                }
                                if ($billing_address_data['phoneNumber'] ?? null) {
                                    $order->set_billing_phone( $billing_address_data['phoneNumber'] );
                                }

                                // update shipping address
                                if ($shipping_address_data['name'] ?? null) {
                                    $order->set_shipping_first_name( $shipping_address_data['name'] );
                                }
                                if ($shipping_address_data['address1'] ?? null) {
                                    $order->set_shipping_address_1( $shipping_address_data['address1'] );
                                }
                                if ($shipping_address_data['address2'] ?? null) {
                                    $order->set_shipping_address_2( $shipping_address_data['address2'] );
                                }
                                if ($shipping_address_data['locality'] ?? null) {
                                    $order->set_shipping_city( $shipping_address_data['locality'] );
                                }
                                if ($shipping_address_data['administrativeArea'] ?? null) {
                                    $order->set_shipping_state( $shipping_address_data['administrativeArea'] );
                                }
                                if ($shipping_address_data['postalCode'] ?? null) {
                                    $order->set_shipping_postcode( $shipping_address_data['postalCode'] );
                                }
                                if ($shipping_address_data['countryCode'] ?? null) {
                                    $order->set_shipping_country( $shipping_address_data['countryCode'] );
                                }
                                if ($shipping_address_data['phoneNumber'] ?? null) {
                                    $order->set_shipping_phone( $shipping_address_data['phoneNumber'] );
                                } elseif ($billing_address_data['phoneNumber'] ?? null) {
                                    $order->set_shipping_phone( $billing_address_data['phoneNumber'] );
                                }
