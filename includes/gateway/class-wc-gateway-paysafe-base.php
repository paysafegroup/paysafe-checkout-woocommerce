...
abstract class WC_Gateway_Paysafe_Base extends WC_Payment_Gateway {
    ...
    const ALLOW_EXPRESS_PAYMENT_GOOGLE_PAY = false;
    const ALLOW_EXPRESS_PAYMENT_GOOGLE_PAY_PRODUCT_PAGE = false;

    const ALLOW_LPM_ACH = false;
    const ALLOW_LPM_EFT = false;
    const ALLOW_LPM_PAY_BY_BANK = false;
    const ALLOW_LPM_PAYPAL = false;
    ...
    private function get_payment_gateway_public_name(string $gateway_code): string
    {
        return match ($gateway_code) {
            ...
            'sightline', 'SIGHTLINE' => __('Sightline', 'paysafe-checkout'),
            'vippreferred', 'VIPPREFERRED' => __('Vippreferred', 'paysafe-checkout'),
            'pay_by_bank', 'PAY_BY_BANK' => __('Paybybank', 'paysafe-checkout'),
    ...
    public function handle_paysafe_product_express_ag_pay_checkout(): void
    {
        ...
        try {
            ...
            if ( ! wp_verify_nonce( $nonce, 'paysafe_express_ag_pay_product_checkout' ) ) {
                throw new Exception(
