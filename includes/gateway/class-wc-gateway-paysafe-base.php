...
abstract class WC_Gateway_Paysafe_Base extends WC_Payment_Gateway {
    ...
    // Allow a customer to be linked to a guest order?
    // customer is searched by billing email
    // - if found, the customer is linked to the order
    // - if not found, the customer is created then linked to the order
    const ALLOW_GUEST_ORDERS_LINK_TO_CUSTOMER = false;
    ...
    const ALLOW_LPM_VIPPREFERRED = true;
    const ALLOW_LPM_VENMO = true;
    ...
    public function __construct() {
        ...
        add_action(
            ...
            [
                $this,
                'handle_paysafe_checkout_express_ag_pay_checkout'
            ]
        );

        // register the express apple/google pay endpoint for checkout page
        add_action(
            'woocommerce_api_paysafe_checkout_express_ag_pay_checkout',
            [
                $this,
                'handle_paysafe_checkout_express_ag_pay_checkout'
            ]
        ...
        // remove AP & GP from the legacy payment list, if its set as express payment
	    add_filter(
                'woocommerce_available_payment_gateways',
                [
                        $this,
                    'paysafe_woo_express_legacy_gateway_filter'
                ],
                40
        );
	 ...
	 */
    public function enqueue_admin_react_app(): void
    {
    ...
    ): array
    {
        ...
        $return = [
            ...
            'apple_pay_issuer_country' => $this->settings[ 'apple_pay_issuer_country' . $this->test_live_suffix() ] ?? '',
            'google_pay_issuer_country' => $this->settings[ 'google_pay_issuer_country' . $this->test_live_suffix() ] ?? '',
            'google_pay_merchant_id' => $this->settings[ 'google_pay_merchant_id' . $this->test_live_suffix() ] ?? '',
        ];
    ...
    public function enqueue_paysafe_scripts()
    {
        ...
        wp_register_script(
	        $this->id . '-paysafe-checkout',
            $script_assets_base_url . 'checkout/v2/paysafe.checkout.min.js',
            [],
            time(),
            true
        );
        wp_enqueue_script($this->id . '-paysafe-checkout');
        ...
        wp_enqueue_script($this->id . '-paysafe-js');
	    ...
	    if (
            ...
            wp_register_script(
                $this->id . '-paysafe-legacy-checkout',
                PAYSAFE_WOO_PLUGIN_URL . 'assets/js/legacy/legacy-paysafe.js',
	            ['wp-i18n'],
                PAYSAFE_WOO_VERSION,
                true,
            );
            wp_localize_script(
                $this->id . '-paysafe-legacy-checkout',
                'paysafe_legacy_settings',
                array_merge($base_settings, $checkout_settings)
            );
            wp_enqueue_script($this->id . '-paysafe-legacy-checkout');
        ...
        if (is_add_payment_method_page() && $this->is_save_token_enabled()) {
            ...
            wp_register_script(
                $this->id . '-paysafe-apm',
                PAYSAFE_WOO_PLUGIN_URL . 'assets/js/legacy/add-payment-method.js',
	            ['wp-i18n'],
                PAYSAFE_WOO_VERSION,
                true,
            );
            wp_localize_script(
                $this->id . '-paysafe-apm',
                'paysafe_apm_settings',
                array_merge($base_settings, $apm_settings)
            );
            wp_enqueue_script($this->id . '-paysafe-apm');
        ...
        if (is_checkout_pay_page()) {
            ...
            wp_register_script(
                $this->id . '-paysafe-op',
                PAYSAFE_WOO_PLUGIN_URL . 'assets/js/legacy/my-account-order-pay.js',
                ['wp-i18n'],
                PAYSAFE_WOO_VERSION,
                true,
            );
            ...
            wp_enqueue_script($this->id . '-paysafe-op');
    ...
    public function handle_payment_response(): void
    {
        ...
        if ( ! is_array( $response ) ) {
            wp_send_json( [ 'status' => 'error', 'error_message' => 'Invalid request.' ], 400 );
            wp_die();
        }
        $response = wc_clean( $response );
        ...
        if ($order && $payment_handle_token) {
            ...
            // if order has no billing address, get the billing address from the payment handle data
            if (in_array($paysafe_payment_method, [
                    WC_Gateway_Paysafe_Apple_Pay::PAYMENT_TYPE_CODE,
                    WC_Gateway_Paysafe_Google_Pay::PAYMENT_TYPE_CODE,
                    ])
            ) {
                $this->update_order_billing_and_shipping_by_payment_handle(
                    $order,
                    $merchant_ref_num,
            ...
            if ($is_subscription) {
	            ...
	            if ($success) {
                    ...
                    $multi_use_token = $this->save_card_and_return_mut($payment_handle_token);
		            update_post_meta(
                        $original_order_id,
        ...
        if (!empty($settlement_id) && in_array($status, $accepted_statuses, true)) {
            ...
            $order->add_order_note(
                sprintf(
                    ...
                    ' <br />' .
                    '<b>' . __('Settlement ID:', 'paysafe-checkout') . '</b>' .
                    ' %2$s.' . "\n\n" .
                    __(
                    ...
                    __(
                        'Refunds are available only after the payment has been captured and fully settled.',
                        'paysafe-checkout'
                    ),
            ...
            $order->add_order_note(
                sprintf(
                    __('Payment is processing.', 'paysafe-checkout') . ' <br />' .
                    '<b>' . __('Payment ID:', 'paysafe-checkout') . '</b> ' .
                    ' %s.' . "\n\n" .
                    __(
            ...
            if (!$capture_payment) {
                ...
                $order->add_order_note(
                    sprintf(
                        ...
                        __('Payment authorization of %s successfully completed.', 'paysafe-checkout') . ' <br />' .
                        '<b>' . __('Authorization ID:', 'paysafe-checkout') . '</b> ' .
                        ' %s.' . "\n\n" .
                        __('To complete the payment, please change the order status to “Processing” - this will capture the authorized amount.', 'paysafe-checkout') . ' <br />' .
                        __('If you want to capture only part of the authorized amount, please refer to your Merchant Manual.', 'paysafe-checkout') . ' <br />' .
                        __(
                        ...
                        '<b>' . __('Note:', 'paysafe-checkout') . '</b> ' .
                        __(
                            'Refunds are only available after the payment has been captured and fully settled.',
                            'paysafe-checkout'
                        ),
    ...
    public function handle_order_status_edited(int $order_id, string $status_to = null): void
    {
        ...
        if ($status_to === self::ORDER_STATUS_REFUNDED && $order->get_remaining_refund_amount() > 0) {
            $order->add_order_note(
                __('The order status has been changed to Refunded.', 'paysafe-checkout') . ' <br />' .
                __(
        ...
        if (
            ...
            if (self::PAYSAFE_RESPONSE_STATUS_COMPLETED === $this->get_settlement_status($settlement_id)) {
                ...
                if ($cancel_success) {
                    $order->add_order_note(
                        __('The order status has been changed to Cancelled.', 'paysafe-checkout') . ' <br />' .
                        sprintf(
    ...
    public function handle_delete_paysafe_customer_data(): void
    {
	    if ( ! isset( $_GET['_wpnonce'] )
		    ...
		    wp_die( esc_html__( 'Security check failed', 'paysafe-checkout' ) );
	    }
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        try {
            ...
            $refund_message = __('Refunded', 'paysafe-checkout');
            if ($amount < $total_amount) {
                // partial refund
                $refund_message = __('Partially refunded', 'paysafe-checkout');
            }
            $refund_message .= ' %1$s %2$s.<br />';
            $refund_message .= '<b>' . __('Refund ID:', 'paysafe-checkout') . '</b>';
            $refund_message .= ' %3$s';
            $this->log_debug(
                self::PAYSAFE_ORDER_ID_KEYWORD
                . $order_id
	        ...
	        $order->add_order_note(
		        sprintf(
			        ($is_partial_settlement ?
				        ...
				        __('However, this is not an issue - the unused portion of the funds will be automatically released by the customer’s bank.', 'paysafe-checkout') . ' <br />' .
            ...
            if ($status === self::PAYSAFE_RESPONSE_STATUS_RECEIVED) {
                ...
                $order->add_order_note(
                    sprintf(
                        ...
                        __('Funds capture of %s started.', 'paysafe-checkout') . ' <br />' .
                        '<b>' . __('Settlement ID:', 'paysafe-checkout') . '</b> %s',
	 ...
	 * @param array|null $sut_details
	 *
	 * @return string|null
	 */
    private function get_paysafe_single_use_card_token(string $paysafe_multi_use_token, array $sut_details = null): ?string
    {
        if ($sut_details === null) {
            return null;
        }
    ...
    public function handle_paysafe_log_error_data(): void
    {
        ...
        // Verify nonce — the frontend JS receives this nonce from get_paysafe_checkout_settings()
        $nonce = $post['nonce'] ?? null;
        if ( ! wp_verify_nonce( $nonce, 'paysafe_payment_response' ) ) {
            wp_send_json( [ 'status' => 'fail' ], 403 );
            wp_die();
        }
    ...
    public function handle_paysafe_express_checkout(bool $is_apple_pay = false): void
    {
        ...
        $order_note = $post['order_comments'] ?? '';
        ...
        try {
            ...
            if (
                ...
                $order_data = [
                    'billing' => [
                        'first_name' => $first_name ?? $current_customer->get_billing_first_name(),
                        'last_name' => $last_name ?? $current_customer->get_billing_last_name(),
                        'email' => $email ?? $current_customer->get_billing_email(),
                        'country' => $country ?? $current_customer->get_billing_country(),
                        'state' => $state ?? $current_customer->get_billing_state(),
                        'postcode' => $postcode ?? $current_customer->get_billing_postcode(),
                        'city' => $city ?? $current_customer->get_billing_city(),
                        'address' => $address_1 ?? $current_customer->get_billing_address(),
                        'address_1' => $address_1 ?? $current_customer->get_billing_address(),
                        'address_2' => $address_2 ?? $current_customer->get_billing_address_2(),
                        'phone' => $phone ?? $current_customer->get_billing_phone(),
                    ],
                    'shipping' => [
                        'first_name' => $first_name ?? $current_customer->get_shipping_first_name(),
                        'last_name' => $last_name ?? $current_customer->get_shipping_last_name(),
                        'country' => $country ?? $current_customer->get_shipping_country(),
                        'state' => $state ?? $current_customer->get_shipping_state(),
                        'postcode' => $postcode ?? $current_customer->get_shipping_postcode(),
                        'city' => $city ?? $current_customer->get_shipping_city(),
                        'address' => $address_1 ?? $current_customer->get_shipping_address(),
                        'address_1' => $address_1 ?? $current_customer->get_shipping_address(),
                        'address_2' => $address_2 ?? $current_customer->get_shipping_address_2(),
                        'phone' => $phone ?? $current_customer->get_shipping_phone(),
                    ],
            ...
            if ($is_apple_pay || in_array($gateway_id, [
                    WC_Gateway_Paysafe_Apple_Pay::PAYMENT_GATEWAY_CODE,
                    WC_Gateway_Paysafe_Google_Pay::PAYMENT_GATEWAY_CODE,
                    ], true)) {
    ...
    /**
     * Get the selected account specific merchant id for a payment gateway
     *
     * @return string
     */
    public function get_account_merchant_id(): string
    {
        return '';
    }
        ...
        switch ($event_name) {
            ...
            case 'SETTLEMENT_COMPLETED':
                ...
                if (in_array($order_current_status, [
                    ...
                    $order->add_order_note(
                        sprintf(
                            __('Payment successfully completed.', 'paysafe-checkout') . ' <br />' .
                            '<b>' . __('Payment ID:', 'paysafe-checkout') . '</b> %s. ' .
                            __(
    ...
    public function handle_webhook_payload(): void
    {
        ...
        $request_data = json_decode($request_string, true);
        if ( ! is_array( $request_data ) ) {
            wp_send_json( [ 'status' => 'FAIL' ], 400 );
            wp_die();
        }
        $request_data = wc_clean( $request_data );
    ...
    protected function is_valid_webhook_signature(string $signature, string $request_string): bool
    {
        ...
        return hash_equals( base64_encode($hmac_payload_string), $signature );
    }
     ...
     * @return void
     */
    public function handle_paysafe_checkout_express_ag_pay_checkout(): void
    {
        $status = 'success';
        $message = '';
	...
	/**
     * Save card token to user and return the Multi Use Token for subscription
     *
	 * @param string $single_use_token
	 *
	 * @return string|null
	 */
    private function save_card_and_return_mut(string $single_use_token): ?string
    {
	    // exchange the single use payment handle that we received from paysafe js
	    // into a multi use payment handle
	    $multi_use_token_data = $this->handle_suph_to_muph_exchange($single_use_token);
    ...
    /**
     * Filter the available payment gateways to hide/show the Apple/Google Pay Express gateways based on the settings
     *
     * @param array $available_gateways
     *
     * @return array
     */
	public function paysafe_woo_express_legacy_gateway_filter($available_gateways): array
	{
		if (is_checkout()) {
			foreach ($available_gateways as $gateway_index => $gateway_data) {
				if (in_array($gateway_index, [
					WC_Gateway_Paysafe_Apple_Pay::PAYMENT_GATEWAY_CODE,
					WC_Gateway_Paysafe_Google_Pay::PAYMENT_GATEWAY_CODE,
				], true)) {

					$settings = $gateway_data->settings;
					$sfx = $this->test_live_suffix();
					if (
						($settings[$gateway_index . '_integration_type' . $sfx] ?? null) === WC_Gateway_Paysafe_Base::PAYMENT_INTEGRATION_PAYSAFE_JS
					) {
						unset($available_gateways[$gateway_index]);
					}
				}
			}
		}
    ...
    /**
     * Return paysafe payment handle data array
     * by given merchant reference number
     *
     * @param string $merchant_ref_num
     *
     * @return array
     * @throws PaysafeException
     */
    public function get_payment_handle_billing_address(string $merchant_ref_num): array
    {
        $api_connector = new PaysafeApiCardPluginConnector();
        return $api_connector->getPaysafePaymentHandleData($merchant_ref_num);
    }

    /**
     * Update the billing and shipping address from payment handle data object
     *
     * @param WC_Order $order
     * @param string $merchant_ref_num
     * @param string $payment_handle_token
     * @param string $paysafe_payment_method
     *
     * @return void
     */
    private function update_order_billing_and_shipping_by_payment_handle(
        WC_Order $order,
        string $merchant_ref_num,
        string $payment_handle_token,
        string $paysafe_payment_method
    ): void
    {
        $order_id = $order->get_id();

        try {
            $payment_handle_data = $this->get_payment_handle_billing_address( $merchant_ref_num );
        } catch (PaysafeException $e) {
            $this->log_debug(
                    self::PAYSAFE_ORDER_ID_KEYWORD
                    . $order_id
                    . '] Paysafe Checkout: Failed to get payment handle data for billing and shipping details',
                    [
                            'error_message' => $e->getMessage(),
                    ]
            );
        ...
        if ($payment_handle_data && isset($payment_handle_data['paymentHandles'])) {
            foreach ($payment_handle_data['paymentHandles'] as $payment_handle_item_data) {
                if ($payment_handle_item_data['paymentHandleToken'] === $payment_handle_token) {
                    // GOT IT !!!
                    $billing_address = $shipping_address = [];
                    $profile = $payment_handle_item_data['profile'] ?? [];

                    if ($paysafe_payment_method === WC_Gateway_Paysafe_Apple_Pay::PAYMENT_TYPE_CODE) {
                        if (isset($payment_handle_item_data['applePay']['applePayPaymentToken'])) {
                            $billing_address = $payment_handle_item_data['applePay']['applePayPaymentToken']['billingContact'] ?? [];
                            $shipping_address = $payment_handle_item_data['applePay']['applePayPaymentToken']['shippingContact'] ?? [];

                            // normalization
                            $billing_address['first_name'] = $billing_address['givenName'] ?? '';
                            $billing_address['last_name'] = $billing_address['familyName'] ?? '';
                            $billing_address['address1'] = $billing_address['addressLines'][0] ?? '';
                            $billing_address['address2'] = $billing_address['addressLines'][1] ?? '';
                            $shipping_address['first_name'] = $shipping_address['givenName'] ?? '';
                            $shipping_address['last_name'] = $shipping_address['familyName'] ?? '';
                            $shipping_address['address1'] = $shipping_address['addressLines'][0] ?? '';
                            $shipping_address['address2'] = $shipping_address['addressLines'][1] ?? '';
                        }
                    }
                    if ($paysafe_payment_method === WC_Gateway_Paysafe_Google_Pay::PAYMENT_TYPE_CODE) {
                        if (isset($payment_handle_item_data['googlePay']['googlePayPaymentToken'])) {
                            $billing_address = $payment_handle_item_data['googlePay']['googlePayPaymentToken']['paymentMethodData']['info']['billingAddress'] ?? [];
                            $shipping_address = $payment_handle_item_data['googlePay']['googlePayPaymentToken']['shippingAddress'] ?? [];
                    ...
                    $this->do_update_order_with_billing_and_shipping(
                            $order,
                            $billing_address,
                            $shipping_address,
                            $profile
    ...
    /**
     * Update the order with billing and shipping information
     *
     * @param WC_Order $order
     ...
     * @return void
     */
    private function do_update_order_with_billing_and_shipping(
        WC_Order $order,
    ...
    ): void
    {
        ...
        try {
            ...
            if ( $profile['email'] ?? null ) {
                $order->set_billing_email( $profile['email'] );
            }

            // update billing address
            if ( $billing_address['first_name'] ?? null ) {
                $order->set_billing_first_name( $billing_address['first_name'] );
            }
            if ( $billing_address['last_name'] ?? null ) {
            ...
            if ( $billing_address['locality'] ?? null ) {
                $order->set_billing_city( $billing_address['locality'] );
            }
            if ( $billing_address['administrativeArea'] ?? null ) {
                $order->set_billing_state( $billing_address['administrativeArea'] );
            }
            if ( $billing_address['postalCode'] ?? null ) {
                $order->set_billing_postcode( $billing_address['postalCode'] );
            }
            if ( $billing_address['countryCode'] ?? null ) {
                $order->set_billing_country( $billing_address['countryCode'] );
            }
            if ( $billing_address['phoneNumber'] ?? null ) {
                $order->set_billing_phone( $billing_address['phoneNumber'] );
            }

            // update shipping address
            if ( $shipping_address['first_name'] ?? null ) {
                $order->set_shipping_first_name( $shipping_address['first_name'] );
            }
            if ( $shipping_address['last_name'] ?? null ) {
            ...
            if ( $shipping_address['locality'] ?? null ) {
                $order->set_shipping_city( $shipping_address['locality'] );
            }
            if ( $shipping_address['administrativeArea'] ?? null ) {
                $order->set_shipping_state( $shipping_address['administrativeArea'] );
            }
            if ( $shipping_address['postalCode'] ?? null ) {
                $order->set_shipping_postcode( $shipping_address['postalCode'] );
            }
            if ( $shipping_address['countryCode'] ?? null ) {
                $order->set_shipping_country( $shipping_address['countryCode'] );
            }
            if ( $shipping_address['phoneNumber'] ?? null ) {
                $order->set_shipping_phone( $shipping_address['phoneNumber'] );
            }

            // add profile data as order note
            $order_note = [];
            if (isset($profile['firstName'])) {
                $order_note[] = trim($profile['firstName'] . ' ' . $profile['lastName']);
            }
            if (isset($profile['email'])) {
                $order_note[] = $profile['email'];
            }
            if (isset($profile['phone'])) {
                $order_note[] = $profile['phone'];
            }
            if (count($order_note) > 0) {
                $order->add_order_note(implode(PHP_EOL, $order_note));
            }

            if ( self::ALLOW_GUEST_ORDERS_LINK_TO_CUSTOMER && $profile['firstName'] ?? null &&  $profile['email'] ?? null && !$order->get_customer_id() ) {
                $customer_id = email_exists( $profile['email'] );
                if ( !$customer_id ) {
                    $customer_id = $this->create_new_customer_with_billing_and_shipping_data(
                        $billing_address,
                        $shipping_address,
                        $profile
                    );
            ...
            $this->log_debug(
                    self::PAYSAFE_ORDER_ID_KEYWORD
                    . $order_id . '] Paysafe Checkout: Error while updating order address from payment handle data',
                    [
                            'error_message' => $e->getMessage(),
                    ]
            );
    ...
    /**
     * Create new customer with billing and shipping data and return the new customer ID
     *
     * @param array $billing_address
     * @param array $shipping_address
     * @param array $profile
     ...
     * @return int|null
     */
    private function create_new_customer_with_billing_and_shipping_data(
        array $billing_address,
        array $shipping_address,
        array $profile
    ): ?int
    {
        if (empty($profile['email'] ?? null) || empty($profile['firstName'] ?? null)) {
            return null;
        }

        // Create the customer and tie it to the order
        $customer = new WC_Customer();

        // Basic info
        $customer->set_email( $profile['email'] ?? '' );
        $customer->set_first_name( $profile['firstName'] ?? '' );
        $customer->set_last_name( $profile['lastName'] ?? '' );
        $customer->set_password( wp_generate_password() ); // or a specific password

        // Billing address
        $customer->set_billing_first_name( $billing_address['first_name'] ?? '' );
        $customer->set_billing_last_name( $billing_address['last_name'] ?? '' );
        $customer->set_billing_email( $profile['email'] ?? '' );
        $customer->set_billing_phone( $profile['phone'] ?? ($billing_address['phoneNumber'] ?? '') );
        $customer->set_billing_address_1( $billing_address['address1'] ?? '' );
        $customer->set_billing_address_2( $billing_address['address2'] ?? '' );
        $customer->set_billing_city( $billing_address['locality'] ?? '' );
        $customer->set_billing_state( $billing_address['administrativeArea'] ?? '' );
        $customer->set_billing_postcode( $billing_address['postalCode'] ?? '' );
        $customer->set_billing_country( $billing_address['countryCode'] ?? '' );
        ...
        $customer->set_shipping_city( $shipping_address['locality'] ?? '' );
        $customer->set_shipping_state( $shipping_address['administrativeArea'] ?? '' );
        $customer->set_shipping_postcode( $shipping_address['postalCode'] ?? '' );
        $customer->set_shipping_country( $shipping_address['countryCode'] ?? '' );

        // Save and return the new customer ID
