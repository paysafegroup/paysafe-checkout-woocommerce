...
class WC_Gateway_Paysafe_Apple_Pay extends WC_Gateway_Paysafe_Base {
    ...
    public function __construct()
    {
		...
		if (self::ALLOW_EXPRESS_PAYMENT_APPLE_PAY_PRODUCT_PAGE) {
		...
		if ($this->is_available()) {
			// Add middleware to add express Apple Pay button to checkout page
			add_action(
				'woocommerce_before_checkout_form',
				[
					$this,
					'handle_add_express_apple_pay_to_checkout_page'
				]
			);
		}
    }
	...
	{
		return $this->settings[ 'apple_pay_issuer_country' . $this->test_live_suffix() ] ?? '';
	}
    ...
    public function handle_add_express_apple_pay_to_product_page(): void
    {
        ...
        if (is_product() && get_current_user_id() && $this->get_integration_type() === self::PAYMENT_INTEGRATION_PAYSAFE_JS) {
            ...
            $apple_pay_product_settings['nonce'] = wp_create_nonce('paysafe_payment_response');


            $script_identifier = 'paysafe-apple-pay-product';
            wp_register_script(
                $script_identifier,
                PAYSAFE_WOO_PLUGIN_URL . 'assets/js/legacy/legacy-paysafe-apple-product.js',
	            ['wp-i18n'],
                PAYSAFE_WOO_VERSION,
                true,
            );
    ...
    /**
     * Add Express Checkout Apple Pay button to checkout page
     *
     * @return void
     */
    public function handle_add_express_apple_pay_to_checkout_page(): void
    {
		if (!$this->is_available()) {
			return;
		}
        ...
        if (is_checkout() && !is_wc_endpoint_url('order-pay') && !is_wc_endpoint_url('order-received') && $this->get_integration_type() === self::PAYMENT_INTEGRATION_PAYSAFE_JS) {
            ...
            // checkout specific settings
            $apple_pay_checkout_settings = $this->get_paysafe_checkout_settings($integration_type);
            $apple_pay_checkout_settings['account_id'] = $this->get_account_id();
            $apple_pay_checkout_settings['express_checkout_url'] = add_query_arg(
                [
                    'payment_gateway'   => $this->id,
                ],
                WC()->api_request_url('paysafe_checkout_express_ag_pay_checkout')
            );
            $apple_pay_checkout_settings['nonce'] = wp_create_nonce('paysafe_payment_response');
	        $apple_pay_checkout_settings['is_apple_pay_express_enabled'] = $this->is_express_gateway_enabled(WC_Gateway_Paysafe_Apple_Pay::PAYMENT_TYPE_CODE);
	        $apple_pay_checkout_settings['is_google_pay_express_enabled'] = $is_google_pay_enabled = $this->is_express_gateway_enabled(WC_Gateway_Paysafe_Google_Pay::PAYMENT_TYPE_CODE);
	        $apple_pay_checkout_settings['express_merchant_ref_num'] = $this->get_merchant_reference_number();
	        $apple_pay_checkout_settings['express_checkout_endpoint_ap'] = add_query_arg(
		        [
			        'payment_gateway'   => $this->id,
		        ],
		        WC()->api_request_url('paysafe_express_checkout')
	        );
	        $apple_pay_checkout_settings['express_checkout_endpoint_gp'] = add_query_arg(
		        [
			        'payment_gateway'   => WC_Gateway_Paysafe_Google_Pay::PAYMENT_GATEWAY_CODE,
		        ],
		        WC()->api_request_url('paysafe_express_checkout')
	        );

            $script_identifier = 'paysafe-apple-pay-checkout-express';
            wp_register_script(
                $script_identifier,
                PAYSAFE_WOO_PLUGIN_URL . 'assets/js/legacy/legacy-paysafe-apple-express.js',
	            ['wp-i18n'],
                PAYSAFE_WOO_VERSION,
                true,
            );
            wp_localize_script(
                $script_identifier,
                'apple_pay_paysafe_config',
                array_merge($base_settings, $apple_pay_checkout_settings)
            );
            wp_enqueue_script($script_identifier);
	        ...
	        $this->show_legacy_express_form($is_google_pay_enabled);
        }
    }
	 ...
	 * Show card details form at checkout
	 *
	 * @param bool $is_google_pay_enabled
	 *
	 * @return void
	 */
	private function show_legacy_express_form(bool $is_google_pay_enabled): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-apple-pay-express-form.php';
