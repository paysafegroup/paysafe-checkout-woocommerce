...
class WC_Gateway_Paysafe_Vippreferred extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'vippreferred';
    const PAYMENT_TYPE_CODE = 'VIPPREFERRED';

    const PAYMENT_ICON_FILE = 'paysafe-vippreferred.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('VIP Preferred', 'paysafe-checkout');
        } else {
            $this->title = __('VIP Preferred', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with VIP Preferred', 'paysafe-checkout');
        }
    ...
    public function payment_fields() {
	    ...
	    $this->show_paysafe_vippreferred_form();
    }

	/**
	 * Show form details at legacy checkout
	 *
	 * @return void
	 */
	private function show_paysafe_vippreferred_form(): void
	{
		ob_start();

		include PAYSAFE_WOO_PLUGIN_PATH . '/resources/html/checkout/legacy-vippreferred-payment-form.php';
