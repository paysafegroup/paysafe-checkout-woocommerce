...
class WC_Gateway_Paysafe_Ach extends WC_Gateway_Paysafe_Base {

    const PAYMENT_GATEWAY_CODE = 'ach';
    const PAYMENT_TYPE_CODE = 'ACH';

    const PAYMENT_ICON_FILE = 'paysafe-ach.svg';
    ...
    public function __construct()
    {
        ...
        if (!empty($this->title)) {
            $this->title .= ' ' . __('ACH', 'paysafe-checkout');
        } else {
            $this->title = __('ACH', 'paysafe-checkout');
        }
        ...
        if (empty($this->description)) {
            $this->description =
                __('Easy and secure payments with ACH', 'paysafe-checkout');
        }
