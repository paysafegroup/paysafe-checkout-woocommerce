    ...
    private static function get_all_payment_methods(array $settings, string $currency_code): array
    {
		...
		if (WC_Gateway_Paysafe_Base::ALLOW_LPM_PAY_BY_BANK) {
			$all_payment_methods[WC_Gateway_Paysafe_Paybybank::PAYMENT_TYPE_CODE] =
				__('Pay by Bank', 'paysafe-checkout');
		}
...
It enhances privacy by minimizing the amount of personal information shared online, as payments are completed using a secure 16-digit code purchased with cash at physical sales locations.',
            ...
            case 'sightline':
                return __(
                    'Play+ (Sightline) provides seamless, secure cashless transactions for gaming and sports betting, connecting digital wallets directly to player\'s accounts.',
                    'paysafe-checkout'
                );
    ...
    private static function build_payment_fields_for_env(
		...
		foreach ($all_payment_methods as $method_code => $payment_method_name) {
			...
			if ($method_code === WC_Gateway_Paysafe_Venmo::PAYMENT_TYPE_CODE) {
				$options['payment_method_' . $currency_code . '_' . $method_code . $sfx]['warning'] =
					__('Venmo is only available in the US and requires additional setup. Please refer to the documentation for more details.', 'paysafe-checkout');
			}
			...
			$options['apple_pay_issuer_country' . $sfx] = [];
			$options['google_pay_issuer_country' . $sfx] = [];
			$options['google_pay_merchant_id' . $sfx] = [];
		}
		...
		if (isset($settings[$envTreeKey][$currency_code])) {
			foreach ($settings[$envTreeKey][$currency_code] as $method_code => $method_data) {
				...
				if ($method_code === WC_Gateway_Paysafe_Apple_Pay::PAYMENT_TYPE_CODE) {
					...
					$options['apple_pay_issuer_country' . $sfx] = [
						'title' => __('Apple Pay settlement issuer country code', 'paysafe-checkout'),
						'type' => 'text',
						'default' => '',
						'description' => __('The 2-letter country code of your Apple Pay merchant account, matching the country/region set for your Apple Merchant ID (for example: US, GB, DE).', 'paysafe-checkout'),
						'custom_attributes' => [
							'autocomplete' => 'off',
						],
						'group' => 'applepay',
				...
				if ($method_code === WC_Gateway_Paysafe_Google_Pay::PAYMENT_TYPE_CODE) {
					...
					$options['google_pay_issuer_country' . $sfx] = [
						'title' => __('Google Pay settlement issuer country code', 'paysafe-checkout'),
						'type' => 'text',
						'default' => '',
						'description' => __('The 2-letter country code of your Google Pay merchant account, matching the country/region set for your Google Merchant ID (for example: US, GB, DE).', 'paysafe-checkout'),
						'custom_attributes' => [
							'autocomplete' => 'off',
						],
						'group' => 'googlepay',
					...
					$options['google_pay_merchant_id' . $sfx] = [
						'title' => __('Google Pay issued Merchant ID', 'paysafe-checkout'),
						'type' => 'text',
						'default' => '',
						'description' => __('Your Google Pay Merchant ID assigned by your payment provider or Google Pay setup, for example: 12345678901234567890.', 'paysafe-checkout'),
						'custom_attributes' => [
							'autocomplete' => 'off',
						],
						'group' => 'googlepay',
