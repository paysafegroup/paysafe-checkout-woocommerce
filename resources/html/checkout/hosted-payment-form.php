...
$paysafe_is_subscription = $this->is_subscription_cart();
$paysafe_terms_page = wc_terms_and_conditions_page_id();
$paysafe_privacy_page = wc_privacy_policy_page_id();
?>
    ...
    <?php if ($this->is_save_token_enabled() && !is_checkout_pay_page()) { ?>
        <div id="save_card" class="wc-block-components-checkbox">
            <label for="paysafe_hosted_save_card">
                <input type="checkbox" class="wc-block-components-checkbox__input" name="paysafe_hosted_save_card" id="paysafe_hosted_save_card" <?php echo $paysafe_is_subscription ? ' checked disabled' : ''; ?> />
        ...
        <?php if ($paysafe_is_subscription) {?>
            <p style="font-size: 0.8em">
                ...
                <?php if ($paysafe_terms_page) { ?>
                    <a href="<?php echo esc_url(get_permalink( $paysafe_terms_page )); ?>"><?php echo esc_html__('Terms and Conditions', 'paysafe-checkout'); ?></a>
                <?php } else { ?>
	                <?php echo esc_html__('Terms and Conditions', 'paysafe-checkout'); ?>
                <?php } ?>
                <?php echo esc_html__('and', 'paysafe-checkout'); ?>
	            <?php if ($paysafe_privacy_page) { ?>
                    <a href="<?php echo esc_url(get_permalink( $paysafe_privacy_page )); ?>"><?php echo esc_html__('Privacy Policy.', 'paysafe-checkout'); ?></a>
	            <?php } else { ?>
