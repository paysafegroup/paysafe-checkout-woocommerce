    ...
    $ ( document ).ready(function() {
        ...
        const log_paysafe_error = function(message, context) {
            if (!!paysafe_apm_settings.log_errors === true && paysafe_apm_settings.log_error_endpoint) {
                const safe_path = validate_paysafe_safe_path(paysafe_apm_settings.log_error_endpoint);
                fetch(
                    safe_path,
                    {
                        method: 'POST',
                        ...
                        body: JSON.stringify({
                            message: message,
                            context: context,
                            nonce: paysafe_apm_settings.nonce || '',
                        }),
                    })
                    .catch((error) => {});
            }
        ...
        paysafe.fields
            ...
            .catch(error => {
                ...
                $('.woocommerce-PaymentBox--paysafe').text(error_message);
            });
        ...
        const validate_paysafe_safe_path = function(endpoint) {
            ...
            if (validated_endpoint.origin !== window.location.origin) {
                $('.woocommerce-PaymentBox--paysafe').text(__('Blocked non-same-origin endpoint', "paysafe-checkout"));
                return;
            }
