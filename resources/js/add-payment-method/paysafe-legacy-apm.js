        ...
        paysafe_form.on(
            ...
            function (e) {
                ...
                if (!paymentInstance || !paymentInstance.areAllFieldsValid()) {
                    const error_message = 'Add payment method failed. Please enter your card details and try again';
                    log_paysafe_error(error_message, []);
                    $('.woocommerce-PaymentBox--paysafe').html(error_message);
                ...
                if (!isHolderNameValid) {
                    const error_message = 'Add payment method failed. Card holder name must have between 2 and 160 characters!';
                    log_paysafe_error(error_message, []);
                    $('.woocommerce-PaymentBox--paysafe').html(error_message);
                ...
                let tokenizationOptions = {
                    ...
                    amount: 0,
                    threeDs: {
                        merchantUrl: paysafe_settings.apm_url,
                    ...
                    paymentType: 'CARD',
                    merchantRefNum: paysafe_settings.merchant_ref_num,
                    customerDetails: {
                        holderName: paysafe_settings.billing_details.name,
                        billingDetails: {
                            country: paysafe_settings.billing_details.country,
                            zip: paysafe_settings.billing_details.zip,
                        },
                        profile: {
                            firstName: paysafe_settings.billing_details.first_name,
                            lastName: paysafe_settings.billing_details.last_name,
                            email: paysafe_settings.billing_details.email
                        },
                    },
                };
                ...
                if (paysafe_settings.billing_details.city) {
                    tokenizationOptions.customerDetails.billingDetails.city = paysafe_settings.billing_details.city;
                }
                if (paysafe_settings.billing_details.state) {
                    tokenizationOptions.customerDetails.billingDetails.state = paysafe_settings.billing_details.state;
                }
                    ...
                    .then(result => {
                        ...
                        paysafe_form.append(
                            $(document.createElement('input'))
                                ...
                                .attr("value", paysafe_settings.merchant_ref_num)
                        );
