...
const init_paysafe_hosted_integration = function() {
    ...
    paysafe.fields
        ...
        .catch(error => {
            ...
            show_paysafe_checkout_error(error_message);
        });
}
                ...
                if (hostedPaymentInstance) {
                    ...
                    if (isTokenizationAvailable) {
                        ...
                        if (isHolderNameValid) {
                            ...
                            log_paysafe_error(log_error_endpoint, __("The payment process failed. Card holder name must have between 2 and 160 characters!", "paysafe-checkout"), {'tokenization_stage': 2});
                            return {
                    ...
                    log_paysafe_error(log_error_endpoint, __("The payment process failed.", "paysafe-checkout"), {'tokenization_stage': 3});
                    return {
                        type: emitResponse.responseTypes.ERROR,
    ...
    useEffect(() => {
        const unsubscribe = onCheckoutSuccess( (response) => {
            ...
            if (orderId && paymentAmount) {
                    ...
                    hostedPaymentInstance
                        ...
                        .then(result => {
                            ...
                            fetch(
                                ...
                                .then(json => {
                                    if (json.status === 'error' || json.error_message) {
                                        show_paysafe_checkout_error(json.error_message);
                                    }
                                ...
                                .catch((error) => {
                                    ...
                                    log_paysafe_error(log_error_endpoint, error_message, []);
                                    show_paysafe_checkout_error(error_message);
                                    show_paysafe_checkout_error(__("The payment process failed. Please reload the page and try again", "paysafe-checkout"), true);
                                });
                        })
                        .catch(error => {
                        ...
                        function (instance, error, result) {
                            if (result && result.paymentHandleToken) {
                                ...
                                fetch(
                                    ...
                                    .then(json => {
                                        if (json.status === 'success') {
                                            ...
                                            log_paysafe_error(log_error_endpoint, __("Payment failed. Showing failure screen.", "paysafe-checkout"), json);
                                    ...
                                    .catch((error) => {
                                        // the BE call failed
                                        log_paysafe_error(log_error_endpoint, __("Payment failed. Showing failure screen.", "paysafe-checkout"), error);
                                        instance.showFailureScreen(__("The payment process failed. Please close this popup and try again", "paysafe-checkout"));
                                    });
                            } else {
                                let errorMessage = '';
                                ...
                                if (instance) {
                                    ...
                                    log_paysafe_error(log_error_endpoint, master_error, {'pace': 4});
                            ...
                            if (stage) {
                                ...
                                switch (stage) {
                                    case "PAYMENT_HANDLE_NOT_CREATED" :
                                        ...
                                        log_paysafe_error(log_error_endpoint, __('Payment failed. Popup was closed without a correct end message!', 'paysafe-checkout'), {'stage': stage, 'expired': expired});
                                ...
                                log_paysafe_error(log_error_endpoint, __('Payment failed. Showing failure screen.', 'paysafe-checkout'), {'stage': stage, 'expired': expired})
                                window.location.reload();
                            }
                        },
                ...
                log_paysafe_error(log_error_endpoint, __("Payment failed. Showing failure screen.", "paysafe-checkout"), {'pace': 5});
            ...
            if (is_cvv_verification) {
                if (cvv_verification_instance) {
                    ...
                    if (!is_tokenization_available) {
                        log_paysafe_error(log_error_endpoint, __("The payment process failed. No CVV entered", "paysafe-checkout"), {'tokenization_stage': 2});
                        return {
                            type: emitResponse.responseTypes.ERROR,
                        ...
                        fetch(
                            ...
                            .then(json => {
                                if (json.error_message) {
                                    // the BE call failed
                                    log_paysafe_error(log_error_endpoint, json.error_message, {'tokenization_stage': 1});
                                    show_paysafe_checkout_error(json.error_message);
                                    show_paysafe_checkout_error(__('The payment process failed. Please reload the page and try again', 'paysafe-checkout'), true);
                                } else {
                                    window.location = json.redirect_url;
                            ...
                            .catch((error) => {
                                // the BE call failed
                                log_paysafe_error(log_error_endpoint, __("The payment process failed.", "paysafe-checkout"), {'tokenization_stage': 1});
                                show_paysafe_checkout_error(__("The payment process failed. Please close this popup and try again", "paysafe-checkout"));
                            });
                    })
                    .catch(error => {
                        // this means that the tokenization of the card form failed,
