...
let saveListenerControllerGooglePay, saveListenerControllerApplePay;
...
const init_apple_pay_hosted_integration = function(onClick, onClose, onError) {
    ...
    if (is_express_apple_pay_initialized) {
        if (document.getElementById('paysafe-apple-pay').firstElementChild !== null) {
            return;
        }
    }
    ...
    if (settings.apple_pay_issuer_country) {
        paysafeOptions.fields.applePay.country = settings.apple_pay_issuer_country;
    }
    ...
    if (is_apple_pay_google_pay_combo_activated) {
        ...
        if (settingsGooglePay.google_pay_issuer_country) {
            paysafeOptions.fields.googlePay.country = settingsGooglePay.google_pay_issuer_country;
        }
        if (settingsGooglePay.google_pay_merchant_id) {
            paysafeOptions.fields.googlePay.merchantId = settingsGooglePay.google_pay_merchant_id;
        }
    }
    ...
    if (currentCartData && currentCartData.billingAddress && currentCartData.billingAddress.postcode) {
        billing_street = currentCartData.billingAddress.address_1;
        billing_street2 = currentCartData.billingAddress.address_2;
        billing_city = currentCartData.billingAddress.city;
        billing_zip = currentCartData.billingAddress.postcode;
        billing_state = currentCartData.billingAddress.state;
        billing_country = currentCartData.billingAddress.country;
    }
    ...
    paysafe.fields
        ...
        .then(paymentMethods => {
            if (paymentMethods.applePay && !paymentMethods.applePay.error) {
                saveListenerControllerApplePay?.abort();
                saveListenerControllerApplePay = new AbortController();
                ...
                document.getElementById('paysafe-apple-pay').addEventListener(
                    ...
                    function (event) {
                        ...
                        if (settings.apple_pay_issuer_country) {
                            hostedTokenizationOptions.applePay.country = settings.apple_pay_issuer_country;
                        }

                        if (billing_country && billing_zip && billing_street && billing_city && billing_state) {
                            hostedTokenizationOptions.customerDetails = {
                                billingDetails: {
                            ...
                            .then(result => {
                                ...
                                fetch(
                                    safe_path,
                                    {
                                        ...
                                        body: JSON.stringify(express_checkout_payload),
                                    })
                                    .then(result => {
                                        return result.json();
                                    })
                    ...
                    { signal: saveListenerControllerApplePay.signal },
                );
            }

            if (is_apple_pay_google_pay_combo_activated && paymentMethods.googlePay && !paymentMethods.googlePay.error) {
                saveListenerControllerGooglePay?.abort();
                saveListenerControllerGooglePay = new AbortController();
                ...
                document.getElementById('paysafe-google-pay').addEventListener(
                    ...
                    function (event) {
                        ...
                        const safe_path = validate_paysafe_safe_path(settingsGooglePay.express_checkout_endpoint);
                        fetch(
                            safe_path,
                            {
                                method: 'POST',
                            ...
                            .then(json => {
                                if (json.status === 'success') {
                                    ...
                                    if (settingsGooglePay.google_pay_issuer_country) {
                                        hostedTokenizationOptions.googlePay.country = settingsGooglePay.google_pay_issuer_country;
                                    }
                                    if (billing_country && billing_zip && billing_street && billing_city && billing_state) {
                                        hostedTokenizationOptions.customerDetails = {
                                            billingDetails: {
                                    ...
                                    hostedApplePayInstance
                                        ...
                                        .then(result => {
                                            ...
                                            const safe_path = validate_paysafe_safe_path(settingsGooglePay.register_url);
                                            fetch(
                                                safe_path,
                                                {
                                                    method: 'POST',
                    ...
                    { signal: saveListenerControllerGooglePay.signal },
                );
            }
        })
        .catch(error => {
