...
export function set_log_nonce(nonce) {
    _log_nonce = nonce || '';
}

export function log_paysafe_error(log_error_endpoint, message, context) {
    if (log_error_endpoint) {
        const endpoint = new URL(log_error_endpoint, window.location.origin);
...
export function get_paysafe_label(title, icon_url) {
    const label = decodeEntities( title ) || __('Paysafe Checkout', 'paysafe-checkout' )
...
export function show_paysafe_checkout_error(error_message, dont_clear_messages) {
    dont_clear_messages = !!dont_clear_messages;
    ...
    if (woo_errors) {
        ...
        // Normalize simple <br> markup to line breaks to avoid HTML injection
        let safeMessage = (error_message || '').replace(/<br\s*\/?>/gi, '\n');

        if (!error_li || dont_clear_messages) {
            error_li.setAttribute('id', 'paysafe-fatal-error-message');
            error_li.textContent = safeMessage;
...
export function main_paysafe_checkout_success_handler(response, currentCartData, settings, options) {
    ...
    let paymentAmount = parseInt(currentCartData.cartTotals.total_price);
    ...
    if (orderId && paymentAmount) {
        ...
        let billing_phone = options && options.additional && options.additional.phone
            ? options.additional.phone : (currentCartData.billingAddress.phone || '');
        ...
        const checkout_options = {
            ...
            displayPaymentMethods: options.displayPaymentMethods,
        ...
        if (customerSingleUseToken) {
            ...
            if (options.customer) {
                checkout_options.customer = options.customer;
            }
        }
        if (!customerSingleUseToken || options.settings.add_customer_when_single_use_token) {
            if (options.customer) {
                checkout_options.customer = options.customer;
            } else {
        ...
        if (!customerSingleUseToken || options.settings.add_billing_when_single_use_token) {
        ...
        if (!!options.settings.use_external_simulator && settings.test_mode) {
            checkout_options.simulator = 'EXTERNAL';
        }

        if (options.shippingAddress) {
            checkout_options.shippingAddress = options.shippingAddress;
        }
...
export function resolve_paysafe_payment_description(settings, addon_description = '') {
    ...
    if (addon_description) {
        descriptionForDisplay.push(addon_description);
    }

    return descriptionForDisplay;
}

export function validate_paysafe_safe_path(endpoint) {
    const validated_endpoint = new URL(endpoint, window.location.href);

    if (validated_endpoint.origin !== window.location.origin) {
        show_paysafe_checkout_error(__('Blocked non-same-origin endpoint', "paysafe-checkout") + " " + validated_endpoint.toString());
        return;
    }
