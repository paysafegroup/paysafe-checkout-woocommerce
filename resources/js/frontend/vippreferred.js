...
const settings = getSetting( 'vippreferred_data', {} );
...
const Content = ( props ) => {
    ...
    useEffect(() => {
        const unsubscribe = onPaymentSetup( async () => {
            ...
            let isVipPreferredConsumerIdValid = vippreferredConsumerIdInput && vippreferredConsumerIdInput.value
                && vippreferredConsumerIdInput.value.length && vippreferredConsumerIdInput.value.length >= 2
                && vippreferredConsumerIdInput.value.length <= 50;
            ...
            log_paysafe_error(__("The payment process failed.  VIP Preferred Social Security Number must be provided!", "paysafe-checkout"), {});
            return {
                type: emitResponse.responseTypes.ERROR,
                message: __("The payment process failed.  VIP Preferred Social Security Number must be provided!", "paysafe-checkout"),
            };
        });
    ...
    useEffect(() => {
        const unsubscribe = onCheckoutSuccess( (response) => {
            ...
            && vippreferredConsumerIdInput.value.length && vippreferredConsumerIdInput.value.length >= 2
            && vippreferredConsumerIdInput.value.length <= 50 ? vippreferredConsumerIdInput.value : '';
    ...
    descriptionForDisplay.push(
        <div id="paysafe-hosted-payment-form" className="paysafe-hosted-payment-form">
            <label id="vippreferred_consumerid_label" className="paysafe-general-form-label"
                   htmlFor="vippreferred_consumeridInput">{__('VIP Preferred Social Security Number', 'paysafe-checkout')}</label>
            <div id="vippreferred_consumerid" className="paysafe-input-field">
                <input type="text" className="paysafe-input-field-input" name="vippreferred_consumerid_input" id="vippreferred_consumeridInput"
                       placeholder={__('Your VIP Preferred Social Security Number', 'paysafe-checkout')} min="2"
...
if (!is_hosted_integration) {
    registerPaymentMethod(PaysafeVippreferredPayments);
}
