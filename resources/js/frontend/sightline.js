...
const settings = getSetting( 'sightline_data', {} );
...
const Content = ( props ) => {
    ...
    useEffect(() => {
        const unsubscribe = onPaymentSetup( async () => {
            ...
            log_paysafe_error(__("The payment process failed. Sightline/Play+ consumer id must have between 2 and 50 characters!", "paysafe-checkout"), {});
            return {
                type: emitResponse.responseTypes.ERROR,
                message: __("The payment process failed. Sightline/Play+ consumer id must have between 2 and 50 characters!", "paysafe-checkout"),
            };
        });
    ...
    descriptionForDisplay.push(
        <div id="paysafe-hosted-payment-form" className="paysafe-hosted-payment-form">
            <label id="sightline_consumerid_label" className="paysafe-general-form-label"
                   htmlFor="sightline_consumeridInput">{__('Sightline/Play+ consumer ID', 'paysafe-checkout')}</label>
            <div id="sightline_consumerid" className="paysafe-input-field">
                <input type="text" className="paysafe-input-field-input" name="sightline_consumerid_input" id="sightline_consumeridInput"
                       placeholder={__('Your Sightline/Play+ consumer ID', 'paysafe-checkout')} min="2"
                       minLength="2" max="50" maxLength="50"/>
            </div>
            <p id="sightline_consumerid_spacer" className="paysafe-general-form-spacer">{__('Enter the consumer id from your Sightline/Play+ account', 'paysafe-checkout')}</p>
        </div>
    );
...
if (!is_hosted_integration) {
    registerPaymentMethod(PaysafeSightlinePayments);
}
