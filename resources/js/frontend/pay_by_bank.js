...
const settings = getSetting( 'pay_by_bank_data', {} );
...
const Content = ( props ) => {
    ...
    useEffect(() => {
        const unsubscribe = onPaymentSetup( async () => {
            ...
            log_paysafe_error(__("The payment process failed. Pay By Bank consumer id must be set!", "paysafe-checkout"), {});
            return {
                type: emitResponse.responseTypes.ERROR,
                message: __("The payment process failed. Pay By Bank consumer id must be set!", "paysafe-checkout"),
            };
        });
    ...
    descriptionForDisplay.push(
        <div id="paysafe-hosted-payment-form" className="paysafe-hosted-payment-form">
            <label id="paybybank_consumerid_label" className="paysafe-general-form-label"
                   htmlFor="paybybank_consumeridInput">{__('Pay By Bank consumer ID', 'paysafe-checkout')}</label>
            <div id="paybybank_consumerid" className="paysafe-input-field">
                <input type="text" className="paysafe-input-field-input" name="paybybank_consumerid_input" id="paybybank_consumeridInput"
                       placeholder={__('Your Pay By Bank consumer ID', 'paysafe-checkout')} min="2"
                       minLength="2" />
            </div>
...
if (!is_hosted_integration) {
    registerPaymentMethod(PaysafePaybybankPayments);
}
