    ...
    $ ( document ).ready(function() {
        ...
        const log_paysafe_error = function(message, context) {
            if (!!paysafe_op_settings.log_errors === true && paysafe_op_settings.log_error_endpoint) {
                const safe_path = validate_paysafe_safe_path(paysafe_op_settings.log_error_endpoint);
                fetch(
                    safe_path,
                    {
                        method: 'POST',
            ...
            function(e) {
                ...
                if (is_skrill) {
                    ...
                    if (!isPaysafeSkrillConsumerIdValid) {
                        show_op_error_message(__("The payment process failed. Skrill email address must be provided!", 'paysafe-checkout'), true);
                        are_fields_valid = false;
                    }
                }
                ...
                const safe_path = validate_paysafe_safe_path(paysafe_op_settings.get_order_pay_data_url);
                fetch(
                    safe_path,
                    {
                        method: 'POST',
                    ...
                    if (use_hosted_integration || is_token_pay) {
                        ...
                        hostedOpPaymentInstance
                            ...
                            .then(result => {
                                ...
                                const safe_path = validate_paysafe_safe_path(paysafe_op_settings.register_url);
                                fetch(
                                    safe_path,
                                    {
                                        method: 'POST',
                        ...
                        if (is_apple_pay) {
                            ...
                            if (paysafe_op_settings.apple_pay_issuer_country) {
                                paymentMethodDetails.applePay.country = paysafe_op_settings.apple_pay_issuer_country;
                            }
                        }
                        ...
                        if (is_google_pay) {
                            ...
                            if (paysafe_op_settings.google_pay_issuer_country) {
                                paymentMethodDetails.googlePay.country = paysafe_op_settings.google_pay_issuer_country;
                            }
                            if (paysafe_op_settings.google_pay_merchant_id) {
                                paymentMethodDetails.googlePay.merchantId = paysafe_op_settings.google_pay_merchant_id;
                            }
                        }
                        ...
                        paysafe.checkout.setup(
                            ...
                            function (instance, error, result) {
                                if (result && result.paymentHandleToken) {
                                    ...
                                    if (instance) {
                                        log_paysafe_error(__("Payment failed. Showing failure screen." + ' ' + errorMessage, 'paysafe-checkout'), {'pace': 3});
        ...
        const show_op_error_message = function(message, clear) {
            ...
            new_error_message.textContent = message;
                ...
                if (!isDefaultValueSet4Skrill) {
                    ...
                    setTimeout(() => {
                        ...
                        if (!!paysafeSkrillConsumerIdInput) {
                            paysafeSkrillConsumerIdInput.value = paysafe_op_settings.consumer_id;
                        }
                    }, 100);
                }
            }
        ...
        const validate_paysafe_safe_path = function(endpoint) {
            ...
            if (validated_endpoint.origin !== window.location.origin) {
                show_op_error_message(__('Blocked non-same-origin endpoint', "paysafe-checkout") + ' ' + validated_endpoint.toString());
                return;
            }
