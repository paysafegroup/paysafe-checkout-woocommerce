(function ( $ ) {
    $ ( document ).ready(function() {
        ...
        const log_paysafe_error = function(message, context) {
            if (!!paysafe_legacy_settings.log_errors === true && paysafe_legacy_settings.log_error_endpoint) {
                const safe_path = validate_paysafe_safe_path(paysafe_legacy_settings.log_error_endpoint);
                fetch(
                    safe_path,
                    {
                        method: 'POST',
            ...
            function(e, wc_checkout_form) {
                ...
                if (is_skrill) {
                    ...
                    if (!isPaysafeSkrillConsumerIdValid) {
                        wc_checkout_form.submit_error('<div class="woocommerce-error">' + __("The payment process failed. Skrill email address must be provided!", "paysafe-checkout") + "</div>");
                        are_fields_valid = false;
                    }
                }
                if (is_sightline) {
                                    ...
                                    if (use_hosted_integration || is_token_pay) {
                                        ...
                                        hostedLegacyPaymentInstance
                                            ...
                                            .then(result => {
                                                ...
                                                const safe_path = validate_paysafe_safe_path(paysafe_legacy_settings.register_url);
                                                fetch(
                                                    safe_path,
                                                    {
                                                        method: 'POST',
                                        ...
                                        if (is_apple_pay) {
                                            ...
                                            if (paysafe_legacy_settings.apple_pay_issuer_country) {
                                                paymentMethodDetails.applePay.country = paysafe_legacy_settings.apple_pay_issuer_country;
                                            }
                                        }
                                        ...
                                        if (is_google_pay) {
                                            ...
                                            if (paysafe_legacy_settings.google_pay_issuer_country) {
                                                paymentMethodDetails.googlePay.country = paysafe_legacy_settings.google_pay_issuer_country;
                                            }
                                            if (paysafe_legacy_settings.google_pay_merchant_id) {
                                                paymentMethodDetails.googlePay.merchantId = paysafe_legacy_settings.google_pay_merchant_id;
                                            }
                                        }
                                        ...
                                        paysafe.checkout.setup(
                                            ...
                                            function (instance, error, result) {
                                                if (result && result.paymentHandleToken) {
                                                    ...
                                                    fetch(
                                                        ...
                                                        .then(json => {
                                                            if (json.status === 'success') {
                                                                ...
                                                                log_paysafe_error(__("Payment failed. Showing failure screen.", "paysafe-checkout"), json);
                                                        ...
                                                        .catch((error) => {
                                                            // the BE call failed
                                                            log_paysafe_error(__("Payment failed. Showing failure screen.", "paysafe-checkout"), error);
                                                            instance.showFailureScreen(__("The payment process failed. Please close this popup and try again", "paysafe-checkout"));
                                                        });
                                                } else {
                                                    let errorMessage = '';
                                                    ...
                                                    if (instance) {
                                                        log_paysafe_error(__("Payment failed. Showing failure screen.", "paysafe-checkout") + ' ' + errorMessage, {'pace': 3});
                                                        ...
                                                        log_paysafe_error(__("Payment failed. Showing failure screen.", "paysafe-checkout") + ' ' + errorMessage, {'pace': 4});
                                            ...
                                            function (stage, expired) {
                                                if (stage) {
                                                    ...
                                                    log_paysafe_error(__("Payment failed. Showing failure screen.", "paysafe-checkout"), {'pace': 4});
                                                    window.location.reload();
                                                }
                                            },
        ...
        let isDefaultValueSet4Skrill = false;
        ...
        const handle_paysafe_js_initialization = function() {
            ...
            if (payment_method === 'skrill') {
                if (!isDefaultValueSet4Skrill) {
                    isDefaultValueSet4Skrill = true;
                    setTimeout(() => {
                        let paysafeSkrillConsumerIdInput = document.getElementById('paysafeSkrill_consumeridInput');
                        if (!!paysafeSkrillConsumerIdInput) {
                            paysafeSkrillConsumerIdInput.value = paysafe_legacy_settings.consumer_id;
                        }
                    }, 100);
                }
            }
    ...
    // todo add error message displaying method
}) ( jQuery );
