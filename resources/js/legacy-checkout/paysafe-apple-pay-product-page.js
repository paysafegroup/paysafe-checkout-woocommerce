(function( $ ) {
    ...
    const log_paysafe_error = function(message, context) {
        if (apple_pay_paysafe_pp_config.log_errors === true && apple_pay_paysafe_pp_config.log_error_endpoint) {
            const safe_path = validate_paysafe_safe_path(apple_pay_paysafe_pp_config.log_error_endpoint);
            fetch(
                safe_path,
                {
                    method: 'POST',
    ...
    const init_express_apple_pay_on_product_page = function() {
        ...
        const paysafeOptions = {
            ...
            currencyCode: apple_pay_paysafe_pp_config.currency_code,

            // select the Paysafe test / sandbox environment
            environment: apple_pay_paysafe_pp_config.test_mode ? 'TEST' : 'LIVE',

            transactionSource: 'WooCommerceJs',

            accounts: {
                default: parseInt(apple_pay_paysafe_pp_config.account_id),
                applePay: parseInt(apple_pay_paysafe_pp_config.account_id),
            },
        ...
        paysafe.fields
            .setup(
                apple_pay_paysafe_pp_config.authorization,
                paysafeOptions
            )
            .then(instance => {
                        ...
                        function (event) {
                            ...
                            if (productCartForm[0].checkValidity()) {
                                ...
                                const register_product_purchase_data = {
                                    ...
                                    nonce: apple_pay_paysafe_pp_config.nonce,
                                    product_data: productCartData,
                                };


                                const safe_path = validate_paysafe_safe_path(apple_pay_paysafe_pp_config.express_checkout_url);
                                fetch(
                                    safe_path,
                                    {
                                        method: 'POST',
                                    ...
                                    .then(json => {
                                        if (json.status === 'success') {
                                            ...
                                            const hostedTokenizationOptions = {
                                                ...
                                                accountId: parseInt(apple_pay_paysafe_pp_config.account_id),
                                                customerDetails: {
                                                    billingDetails: {
                                                        country: billing_country,
                                                        zip: billing_zip,
                                            ...
                                            hostedApplePayInstance
                                                ...
                                                .then(result => {
                                                    ...
                                                    const safe_path = validate_paysafe_safe_path(apple_pay_paysafe_pp_config.register_url);
                                                    fetch(
                                                        safe_path,
                                                        {
                                                            method: 'POST',
